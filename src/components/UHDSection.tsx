...
export default function UHDSection({ applianceState, setApplianceState, totalLoad, onOpenPlans }: UHDSectionProps) {
  ...
  return (
    <section className="bg-slate-900 py-32 px-12 border-t border-white/5 relative overflow-hidden">
      {/* Background Ambient Glow - Cyber Aqua */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[500px] bg-accent-cyber/5 blur-[120px] rounded-full -mt-[250px]" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          <div>
            <div className="flex items-center gap-4 mb-4">
               ...
               <p className="text-white/40 font-mono text-[10px] uppercase tracking-[0.4em] font-black">Control Hub V3.6.0</p>
            </div>
            <h2 className="text-7xl font-black text-white tracking-tighter uppercase mb-4 leading-none">
               Command <span className="text-accent-cyber">Center</span>
            </h2>
          </div>
          
          <div className="flex items-center gap-12 bg-slate-950/50 backdrop-blur-3xl border border-white/5 p-8 rounded-[3rem] shadow-2xl">
            <div className="text-left">
               <p className="text-white/40 text-[10px] uppercase font-mono tracking-widest mb-1 font-bold">Grid Load Meter</p>
               <p className="text-white text-6xl font-black tracking-tighter tabular-nums drop-shadow-[0_0_20px_rgba(0,240,255,0.2)]">
                 {totalLoad.toFixed(2)}<span className="text-lg text-white/20 ml-2">kW</span>
               </p>
            </div>
            <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center border border-white/10 transition-all duration-500 ${totalLoad > 7 ? "bg-red-500 text-white shadow-[0_15px_30px_rgba(239,68,68,0.3)]" : "bg-accent-cyber text-slate-900 shadow-[0_0_30px_rgba(0,240,255,0.4)]"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {appliances.map((app) => (
            ...
            >
              ...
              <div className="text-left relative z-10 w-full">
                ...
                <p className="text-white/40 text-xs font-bold mb-10 uppercase tracking-tighter">{app.desc}</p>
                ...
                <div className="flex items-center justify-between w-full pt-6 border-t border-white/5">
                   <p className={`text-[10px] font-mono tracking-widest uppercase font-black ${
                     applianceState[app.id as keyof ApplianceState] ? "text-accent-cyber" : "text-white/30"
                   }`}>
                     {applianceState[app.id as keyof ApplianceState] ? "ACTIVE" : "STANDBY"}
                   </p>
                   <p className="text-white/50 font-black text-xs">{app.power}</p>
                </div>
              </div>
            </motion.button>
          ))}
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.98 }}
            className="p-10 rounded-[3.5rem] border border-accent-cyber/20 bg-accent-cyber/5 backdrop-blur-md flex flex-col justify-between group shadow-lg text-left"
          >
             <div className="w-16 h-16 rounded-3xl bg-accent-cyber text-slate-900 flex items-center justify-center mb-8 border border-white/10">
                <LayoutGrid className="w-8 h-8" />
             </div>
             <div>
               <p className="text-accent-cyber font-mono text-[10px] uppercase tracking-[0.2em] mb-4 font-black">Optimization Matrix</p>
               <h3 className="text-white font-black text-3xl leading-tight uppercase tracking-tighter">Maximize <br/> your savings.</h3>
             </div>
             <div className="flex items-center gap-4 text-accent-cyber font-black uppercase text-xs tracking-widest mt-8 group-hover:gap-6 transition-all">
