...
export default function BudgetManager({ totalLoad, costFactor }: BudgetManagerProps) {
  ...
  return (
    <div className="bg-slate-900 border border-white/10 rounded-[2rem] p-6 sm:p-8 backdrop-blur-xl relative overflow-hidden group">
      <div className={`absolute inset-0 bg-gradient-to-br ${isOverBudget ? 'from-red-500/5' : 'from-emerald-500/5'} to-transparent opacity-50 transition-colors duration-500`} />
      
      <div className="relative z-10 flex flex-col gap-6">
        <div className="flex justify-between items-center border-b border-white/10 pb-4">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl ${isOverBudget ? 'bg-red-500/10 border-red-500/30 text-red-500' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500'} border flex items-center justify-center transition-colors`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
            ...
            <div className={`text-2xl font-black ${isOverBudget ? 'text-red-400' : 'text-emerald-400'}`}>
              ₹{projectedMonthlyCost.toFixed(0)} <span className="text-sm text-white/40">/mo</span>
            </div>
          </div>
            ...
            {isEditing ? (
              <div className="flex items-center gap-2">
                ...
                <input 
                  ...
                  autoFocus
                  className="bg-transparent border-b border-accent-cyber outline-none text-2xl font-black text-white w-full"
                />
              </div>
            ) : (
              <div className="text-2xl font-black text-white">
                ₹{budgetTarget.toFixed(0)} <span className="text-sm text-white/40">/mo</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <p className={`text-[10px] uppercase font-black tracking-wider ${isOverBudget ? 'text-red-400' : 'text-emerald-400'}`}>
