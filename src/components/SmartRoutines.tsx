...
export default function SmartRoutines({ onExecuteRoutine, activeRoutine }: SmartRoutinesProps) {
  return (
    <div className="w-full mt-12 sm:mt-16 bg-slate-950/80 backdrop-blur-3xl border border-white/5 p-6 sm:p-10 rounded-2xl shadow-xl">
      <div className="flex items-center gap-3 mb-6 sm:mb-8">
        <div className="w-2 h-2 rounded-full bg-accent-routine animate-pulse" />
        <h3 className="text-white font-black uppercase tracking-widest text-base sm:text-xl font-heading">Smart Routines</h3>
      </div>
        ...
        {ROUTINES.map((routine) => {
          ...
          return (
            <motion.button
              ...
              className={`relative overflow-hidden p-4 sm:p-6 rounded-2xl border transition-all duration-300 flex flex-col items-center justify-center gap-3 text-center ${
                isActive 
                  ? "bg-accent-routine/10 border-accent-routine text-white" 
            ...
            >
              ...
              <div>
                <p className="font-black uppercase tracking-tighter text-sm sm:text-base font-heading">{routine.label}</p>
